...
{
	SQLite3DB::Statement st=m_db->Prepare("SELECT tblMessage.MessageID, tblMessage.Subject, tblMessage.FromName, tblMessage.MessageDate || ' ' || tblMessage.MessageTime FROM tblMessageReplyTo INNER JOIN tblMessage ON tblMessageReplyTo.MessageID=tblMessage.MessageID INNER JOIN tblMessageBoard ON tblMessage.MessageID=tblMessageBoard.MessageID WHERE ReplyToMessageUUID=? AND tblMessageBoard.BoardID=?;");
...
const MessageThread::threadnode MessageThread::GetOriginalMessageNode(const long messageid, const long boardid, const unsigned int currentdepth)
{
	...
	else
	{
		...
		SQLite3DB::Statement st2=m_db->Prepare("SELECT Subject, FromName, MessageDate || ' ' || MessageTime FROM tblMessage INNER JOIN tblMessageBoard ON tblMessage.MessageID=tblMessageBoard.MessageID WHERE tblMessage.MessageID=? AND tblMessageBoard.BoardID=?;");
...
const bool MessageThread::Load(const long messageid, const long boardid, const bool bydate)
{
	...
	if(originalmessagenode.m_messageid>=0)
	{
		...
		if(originalmessagenode.m_messageid==messageid)
		{
			...
			SQLite3DB::Statement st=m_db->Prepare("SELECT ReplyToMessageUUID FROM tblMessageReplyTo INNER JOIN tblMessageBoard ON tblMessageReplyTo.MessageID=tblMessageBoard.MessageID WHERE tblMessageReplyTo.MessageID=? AND tblMessageBoard.BoardID=?;");
			st.Bind(0,messageid);
			st.Bind(1,boardid);
			st.Step();
			if(st.RowReturned())
