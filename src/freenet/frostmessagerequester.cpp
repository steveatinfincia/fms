...
void FrostMessageRequester::StartRequest(const std::string &id)
{
	...
	SQLite3DB::Statement st=m_db->Prepare("SELECT BoardName, FrostPublicKey, FrostBoardName FROM tblBoard WHERE BoardID=?;");
	st.Bind(0,idparts[2]);
	st.Step();
	...
	if(st.RowReturned())
	{
		...
		// if there's a frost specific board name, override the boardname with it
		st.ResultText(2,frostboardname);
		if(frostboardname!="")
		{
			boardname=frostboardname;
		}
