...
bool BMP::SetSize(int NewWidth , int NewHeight )
{
 ...
 if( NewWidth >= 32768 || NewHeight >= 32768 )
 {
  if( EasyBMPwarnings )
  {
   cout << "EasyBMP Warning: User attempted to set a large width or height." << endl
        << "                 Size remains unchanged at "
        << Width << " x " << Height << "." << endl;
  }
  return false;
...
{ 
 ...
 if( !SetSize( (int) bmih.biWidth , (int) bmih.biHeight ) )
 {
  SetSize(1,1);
  SetBitDepth(1);
  fclose(fp);
 ...
 if( BitDepth < 16 )
 {
  ...
  if( NumberOfColorsToRead<0 )
  {
   if( EasyBMPwarnings )
   {
    cout << "EasyBMP Warning: file " << FileName << " has negative" << endl
         << "                 color table." << endl;
   }
   SetSize(1,1);
   SetBitDepth(1);
